...
export const ProductsPage = () => {
  ...
  // Sync state if URL changes (e.g., from Navbar clicking "Shop All" or browser back/forward)
  useEffect(() => {
  ...
  const filteredProducts = useMemo(() => {
    let result = products.filter(product => {
      ...
      const matchesSubCategory = selectedSubCategory === 'All' ||
        ((selectedCategory === 'Clothes' || selectedCategory === 'Shoes') && product.name.includes(selectedSubCategory)); // Simple name check for sub-category if field missing
      const matchesPrice = product.price >= priceRange[0] && product.price <= priceRange[1];
      const matchesRating = (product.rating || 0) >= minRating;
      const matchesCompany = selectedCompanies.length === 0 || selectedCompanies.includes(product.company);

      return matchesSearch && matchesCategory && matchesPrice && matchesSubCategory && matchesRating && matchesCompany;
    });
    ...
    return result;
  }, [products, searchQuery, selectedCategory, selectedSubCategory, priceRange, minRating, selectedCompanies, sortBy]);
  ...
  const hasActiveFilters = selectedCategory !== 'All' || priceRange[1] !== maxPrice || !!searchQuery || minRating > 0 || selectedCompanies.length > 0 || sortBy !== 'latest';
  ...
  // How many companies to show initially
  const COMPANIES_TO_SHOW = 5;
  const displayedCompanies = showAllCompanies ? availableCompanies : availableCompanies.slice(0, COMPANIES_TO_SHOW);
          ...
          <div className="hidden md:flex justify-between items-center mb-6">
            ...
            <div id="sort-dropdown-container" className="relative">
              ...
              >
                ...
                <span className="text-foreground">{SORT_OPTIONS.find(o => o.value === sortBy)?.label}</span>
                <ChevronDown className={`h-4 w-4 text-muted-foreground transition-transform duration-200 ${sortDropdownOpen ? 'rotate-180' : ''}`} />
              </button>
          ...
          <label key={category} className="flex items-center space-x-2 cursor-pointer group">
            <input
              ...
              onChange={() => {
                ...
                setSelectedSubCategory('All');
                toggleCompany('All'); // Not correct, but handled in parent reset logic
      ...
      <div className="space-y-2">
        {[0, 4.5, 4, 3].map(rating => (
          <label key={rating} className="flex items-center space-x-2 cursor-pointer group">
            ...
            />
            <span className={`text-sm group-hover:text-yellow-600 transition-colors flex items-center gap-1.5 ${minRating === rating ? 'font-medium text-yellow-600' : 'text-muted-foreground'}`}>
              {rating === 0 ? (
                'All Ratings'
              ) : (
                <>
      ...
      <div className="space-y-2">
        {displayedCompanies.map(company => (
          <label key={company} className="flex items-center space-x-2 cursor-pointer group">
            ...
            />
            <span className={`text-sm group-hover:text-blue-600 transition-colors ${selectedCompanies.includes(company) ? 'font-medium text-blue-600' : 'text-muted-foreground'}`}>
              {company}
            </span>
          </label>
        ))}
        {availableCompanies.length > COMPANIES_TO_SHOW && (
          ...
          >
            {showAllCompanies ? '− Show Less' : `+ ${availableCompanies.length - COMPANIES_TO_SHOW} More`}
          </button>
        )}
      </div>
    </div>
      ...
      <div className="px-2 space-y-4">
        ...
        <div className="flex justify-between items-center text-xs font-medium">
          <div className="flex flex-col">
            <span className="text-muted-foreground uppercase tracking-wider text-[10px]">Min Price</span>
            <span className="text-foreground">₹{priceRange[0].toLocaleString('en-IN')}</span>
          </div>
          <div className="flex flex-col text-right">
            <span className="text-muted-foreground uppercase tracking-wider text-[10px]">Max Price</span>
